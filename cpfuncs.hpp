...
/*int main() {
    ...
    vec<int> nooms = {1, 53, 54, 64, 65, 63, 63, 87};
    vec<string> words = {"one", "two", "three", "four", "five", "six"};
    //auto paired = link(nums, words, full);
    //opp(paired);

    //auto map1 = mapp(nums, words);
    //cout << map1[4] << endl;

    string gs = "1 3 2 5 2 5 2 53 7 3 83 3";
    //int hunt = occur(gs, ' ');
    //cout << hunt << endl;

    auto honn = take<int>(3);
    vp(honn, "\n");
